Termucoin Fee Recommendation
----------------------------

_last updated for 1.14.6_

The Termucoin chain has a relatively low block interval, 1 megabyte blockspace
and aims to provide a cheap means for people to transact. Therefore, the biggest
threat to the Termucoin chain as a whole is spam and in 2014, a transaction fee
and dust disincentive were introduced, to combat on-chain spam.

Termucoin Core implements a number of defaults into the software that reflect the
...
parametrized. The recommended defaults, as implemented in the Termucoin Core
wallet, are:

- **0.01 TERM per kilobyte** transaction fee
- **0.01 TERM** dust limit (discard threshold)
- **0.001 TERM** replace-by-fee increments

The wallet rejects transactions that have outputs under the dust limit, and
discards change to fee if it falls under this limit.

Note: In the past, Termucoin has enforced a rounding function in the fee
      ...
      transaction only has to pay `0.01 / 1000 * 192 = 0.00192` TERM fee.

## Miner default inclusion policies

The default values for miners to include a transaction in a block has been set
to exactly the recommended fee of **0.01 TERM/kB.** Dust limits are defined by
...
The default minimum transaction fee for relay is set at **0.001 TERM/kB**,
...
- The hard dust limit is set at **0.001 TERM** - outputs under this value are
  invalid and rejected.
- The soft dust limit is set at **0.01 TERM** - sending a transaction with outputs
  under this value, are required to add 0.01 TERM for each such output, or else
