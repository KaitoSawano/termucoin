...
Below are some notes on how to build Termucoin Core for Windows.
...
To build Termucoin from source code, you'll need the source code. Either check it out via `git` or download
a zip file. (Look at the green "<> Code" button on [the Termucoin GitHub repository](https://github.com/termucoin/termucoin/)).
...
Note that for WSL v1 the Termucoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/termucoin, AND not under, for example, /mnt/d/termucoin.
...
```
Ensure that this toolchain can build for `posix` (else you may have compilation errors for Termucoin's dependencies):
```bash
...
way. This will install to `c:\workspace\termucoin`, for example:
```bash
make install DESTDIR=/mnt/c/workspace/termucoin
```
