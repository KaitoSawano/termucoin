...
`gitian-build.sh` is a standalone script, it can be downloaded and run outside of Termucoin Core repository.
...
```bash
gpg: skipped "aldianokto": secret key not available
```
...
```bash
gpg --detach-sign ${VERSION}-linux/${SIGNER}/termucoin-linux-build.assert
gpg --detach-sign ${VERSION}-win/${SIGNER}/termucoin-win-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/termucoin-osx-build.assert
```
...
Gitian signatures for each release are added to https://github.com/termucoin/gitian.sigs.

`gitian-build.sh` will create signatures inside `gitian-output/sigs/` folder. Create a pull request to [termucoin/gitian.sigs](https://github.com/termucoin/gitian.sigs) to publish your signatures, the `.assert` and `.assert.sig` files.

**When your PR is merged, you will be recorded for all future history as a *Gitian Builder of Termucoin Core*!**
