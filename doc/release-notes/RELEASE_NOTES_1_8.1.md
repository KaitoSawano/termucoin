# Termucoin Core 1.8.1

Termucoin Core 1.8.1 is primarily a bugfix release, bringing Termucoin Core in
line with Bitcoin 0.9.3. Termucoin Core 1.8.1 also adds in support for printing
...
## Termucoin Core Release Notes

* Translation improvements and corrections
* Disable Transifex based translation process (not used by Termucoin Core)
* Add checkpoints around AuxPoW switchover block (371,337)
* Disable connections from pre-1.8 Termucoin Core clients
* Use HTTPS with TERMchain.info
...
There is a known issue with quantities in JSON used by the RPC interface (https://github.com/termucoin/termucoin/issues/134).
...
This only affects values significantly below 1 TERM, however merchants may wish to round to
the nearest TERM on all transactions for now.
