Termucoin Core version 1.14.6 is now available from:

  <https://github.com/termucoin/termucoin/releases/tag/v1.14.6/>

This is a new minor version release, including important security updates and
changes to network efficiency. All Termucoin Core users - miners, services,
...
To receive notifications about updates, subscribe to the release mailing list:
  ...
  <https://sourceforge.net/projects/termucoin/lists/termucoin-releases>
...
Termucoin network from 1 TERM to 0.01 TERM. The full recommendation can be found
...
1. The wallet will attempt to not create any outputs smaller than 0.03 TERM as
   change if possible.
2. If after signing there is more than 0.01 TERM left in the change output, the
   wallet will keep the output as-is.
3. If the change output drops under 0.01 TERM, the wallet will discard the
   change and instead give it to miners as fee.
4. Any output under 0.01 TERM will not be accepted as valid by default.
...
```
termucoin-cli addnode <address> remove
```
...
```
termucoin-cli liststucktransactions
```
...
```
termucoin-cli setmaxconnections <desired_amount>
```
...
```
termucoin-cli rescan <start_block>
```
...
efforts to make Termucoin more accessible to people that do not speak English.
...
* Improved and updated the [bip list](bips.md) documentation that explains the
  protocol features implemented by Termucoin Core.
