...
#if defined(NDEBUG)
# error "Termucoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Termucoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetTermucoinMinRelayFee(tx, nSize, !fLimitFree))
        {
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // Termucoin: Switch maturity at depth 145,000
...
void ThreadScriptCheck() {
    RenameThread("termucoin-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Termucoin: BIP30 has been active since inception
    ...
    // Termucoin: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetTermucoinBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Termucoin: Disallow AuxPow blocks before it is activated.
    ...
    // Termucoin: Version 2 enforcement was never used
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // Termucoin: We probably want to disable this
    int nLockTimeFlags = 0;
